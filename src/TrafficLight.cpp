...
void TrafficLight::cycleThroughPhases()
{
    ...
    long cycleDuration = distribution(generator); // first cycle duration in ms

    while (true) {
        std::this_thread::sleep_for(std::chrono::milliseconds(1));  // wait for 1 ms to release CPU load
        ...
        if (timeSinceLastUpdate >= cycleDuration) {
            ...
            cycleDuration = distribution(generator); // new cycle duration in ms
        }
